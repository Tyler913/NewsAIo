...
// Create the API settings file if not present
...
ipcMain.handle("get-api-settings", async () => {
    try {
        ...
        console.error("Failed to read API settings file:", error);
...
ipcMain.handle("save-api-settings", async (event, settings) => {
    try {
        ...
        console.error("Failed to save API settings file:", error);
        return false;
    }
});
...
ipcMain.handle("fetch-rss", async (_, url) => {
    ...
    try {
        // Get API settings
        const apiSettings = await ipcMain.handle("get-api-settings");
        ...
        const response = await fetch("http://141.147.74.209:5000/fetch_rss", {
            ...
            body: JSON.stringify({ 
                ...
                openai_api_key: apiSettings.openaiApiKey,
                deepseek_api_key: apiSettings.deepseekApiKey
            }),
        });
